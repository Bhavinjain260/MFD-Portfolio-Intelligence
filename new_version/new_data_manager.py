...
Every parser inserts RAW file data into the exact tables defined in init_db.py.
...
def _count_before_after(conn, table: str) -> tuple:
...
def parse_kfin_folio(file, replace: bool) -> tuple[bool, str, dict]:
    ...
    for _, row in df.iterrows():
        ...
        try:
            ...
            pan_col   = _kf_col(df, "PAN NUMBER", "PAN_NO", "PAN")
...
def render_data_manager():
    ...
    # ─── BSE ─────────────────────────────────────────────────────────────────
    with tab_bse:
        bse_sec = st.radio("Section", ["👤 Client Master", "📋 XSIP Report"],
        ...
        if bse_sec == "👤 Client Master":
            ...
            if rp: st.warning("All clients + banks + nominees will be deleted first.", icon="⚠️")
            else:  st.info("Append mode: existing client_codes skipped.", icon="ℹ️")
            ...
            with get_conn() as conn:
                df_c = pd.read_sql(
                    ...
                    conn)
            if df_c.empty: st.info("No client data yet.")
            else:
            ...
            if rp: st.warning("All XSIP records will be deleted first.", icon="⚠️")
            else:  st.info("Append mode: duplicate xsip_regn_no skipped.", icon="ℹ️")
            if st.button("📤 Import XSIP", key="dm_bse_xsip_btn") and f:
                with st.spinner("Importing…"):
                    ok, msg, preview = parse_bse_xsip(f, rp)
                    (st.success if ok else st.error)(msg)
                    if ok and preview:
                        _metrics(preview, [("rows","Inserted"),("active","Active"),("duplicates","Duplicates"),("skipped","Skipped")])
                    if ok: st.cache_data.clear()
            ...
            with get_conn() as conn:
                df_x = pd.read_sql(
                    ...
                    conn)
            if df_x.empty: st.info("No XSIP data yet.")
            else: st.dataframe(df_x, use_container_width=True, hide_index=True)
    ...
    with tab_cams:
        cams_sec = st.radio("Section",
                            ["📦 AUM", "💼 Brokerage (WBR77)", "📂 Folio (WBR9)", "💳 Transactions (WBR2)", "🔄 SIP (WBR49)"],
        ...
        if cams_sec == "📦 AUM":
            ...
            if st.button("📤 Upload AUM", key="dm_cams_aum_btn") and f:
                with st.spinner("Parsing…"):
                    ...
                    (st.success if ok else st.error)(msg)
                    if ok and preview:
                        _metrics(preview, [("rows","Inserted"),("total_aum","Total AUM"),("duplicates","Duplicates"),("skipped","Skipped")])
                    if ok: st.cache_data.clear()
            st.divider()
            with get_conn() as conn:
                s = pd.read_sql("SELECT rep_date,amc_code,COUNT(*) folios,ROUND(SUM(rupee_bal),2) aum FROM cams_aum GROUP BY rep_date,amc_code ORDER BY rep_date DESC", conn)
            if s.empty: st.info("No CAMS AUM data.")
            else:
            ...
            if st.button("⚠️ Clear CAMS AUM", key="dm_cams_aum_del"):
                ...
                st.warning("Cleared."); st.cache_data.clear()
            ...
            if rp: st.warning("Replace mode.", icon="⚠️")
            else:  st.info("Dedup: trxn_no + folio_no + brokerage_acrual_month", icon="ℹ️")
            if st.button("📤 Upload WBR77", key="dm_cams_brok_btn") and f:
                with st.spinner("Parsing…"):
                    ok, msg, preview = parse_cams_brokerage(f, rp)
                    (st.success if ok else st.error)(msg)
                    if ok and preview:
                        _metrics(preview, [("rows","Inserted"),("total_brokerage","Total Brokerage"),("duplicates","Duplicates"),("skipped","Skipped")])
                        if preview.get("months"): st.info(f"Months: **{', '.join(preview['months'])}**")
                    if ok: st.cache_data.clear()
            st.divider()
            with get_conn() as conn:
                s = pd.read_sql("SELECT brokerage_acrual_month month,COUNT(*) rows,COUNT(DISTINCT folio_no) folios,ROUND(SUM(brkage_amt),4) brokerage FROM cams_brokerage GROUP BY month ORDER BY month DESC", conn)
            if s.empty: st.info("No CAMS brokerage data.")
            ...
            if st.button("📤 Upload WBR9", key="dm_cams_folio_btn") and f:
                with st.spinner("Parsing…"):
                    ...
                    (st.success if ok else st.error)(msg)
                    if ok and preview:
                        _metrics(preview, [("rows","Inserted"),("total_aum","AUM"),("unique_folios","Folios"),("unique_investors","Investors")])
                    if ok: st.cache_data.clear()
            st.divider()
            with get_conn() as conn:
                s = pd.read_sql("SELECT amc_code,COUNT(DISTINCT folio_no) folios,COUNT(DISTINCT pan_no) investors,ROUND(SUM(rupee_bal),2) aum FROM cams_folio GROUP BY amc_code ORDER BY aum DESC", conn)
            if s.empty: st.info("No CAMS folio data.")
            else:
                s["aum"] = s["aum"].apply(format_aum)
                st.dataframe(s, use_container_width=True, hide_index=True)
            ...
            if st.button("📤 Upload WBR2", key="dm_cams_txn_btn") and f:
                with st.spinner("Parsing…"):
                    ...
                    (st.success if ok else st.error)(msg)
                    if ok and preview:
                        _metrics(preview, [("rows","Inserted"),("total_amount","Total Amount"),("folios","Folios"),("schemes","Schemes")])
                    if ok: st.cache_data.clear()
            st.divider()
            with get_conn() as conn:
                s = pd.read_sql("SELECT trade_date,amc_code,COUNT(*) txns,COUNT(DISTINCT folio_no) folios,ROUND(SUM(amount),2) amount FROM cams_transactions GROUP BY trade_date,amc_code ORDER BY trade_date DESC LIMIT 30", conn)
            if s.empty: st.info("No CAMS transaction data.")
            else: st.dataframe(s, use_container_width=True, hide_index=True)
            ...
            if st.button("📤 Upload WBR49", key="dm_cams_sip_btn") and f:
                with st.spinner("Parsing…"):
                    ...
                    (st.success if ok else st.error)(msg)
                    if ok and preview:
                        _metrics(preview, [("rows","Inserted"),("active","Active"),("ceased","Ceased"),("completed","Completed")])
                    if ok: st.cache_data.clear()
            st.divider()
            with get_conn() as conn:
                s = pd.read_sql("SELECT amc_code,status,COUNT(*) sips,COUNT(DISTINCT folio_no) folios,ROUND(SUM(auto_amount),2) amount FROM cams_sip GROUP BY amc_code,status ORDER BY amc_code", conn)
            if s.empty: st.info("No CAMS SIP data.")
            else: st.dataframe(s, use_container_width=True, hide_index=True)
    ...
    with tab_kfin:
        kf_sec = st.radio("Section",
                          ["📦 AUM", "💼 Brokerage (MFSD205)", "📂 Folio (MFSD211)", "💳 Transactions (MFSD201)", "🔄 SIP (MFSD243)"],
        ...
        if kf_sec == "📦 AUM":
            ...
            st.divider()
            with get_conn() as conn:
                s = pd.read_sql("SELECT rep_date,fund_code,COUNT(*) folios,ROUND(SUM(aum),2) aum FROM kfin_aum GROUP BY rep_date,fund_code ORDER BY rep_date DESC", conn)
            if s.empty: st.info("No KFin AUM data.")
            else:
                s["aum"] = s["aum"].apply(format_aum)
                st.dataframe(s, use_container_width=True, hide_index=True)
            ...
            if rp: st.warning("Replace mode.", icon="⚠️")
            else:  st.info("Dedup: transaction_id", icon="ℹ️")
            ...
            st.divider()
            with get_conn() as conn:
                s = pd.read_sql("SELECT from_date month,COUNT(*) rows,COUNT(DISTINCT account_number) folios,ROUND(SUM(brokerage),4) brokerage FROM kfin_brokerage GROUP BY month ORDER BY month DESC", conn)
            if s.empty: st.info("No KFin brokerage data.")
            else: st.dataframe(s, use_container_width=True, hide_index=True)
            ...
            if st.button("📤 Upload MFSD211", key="dm_kf_folio_btn") and f:
                with st.spinner("Parsing…"):
                    ...
                    (st.success if ok else st.error)(msg)
                    if ok and preview:
                        _metrics(preview, [("rows","Inserted"),("unique_folios","Folios"),("unique_investors","Investors"),("skipped","Skipped")])
                    if ok: st.cache_data.clear()
            st.divider()
            with get_conn() as conn:
                s = pd.read_sql("SELECT fund_code,COUNT(DISTINCT folio_no) folios,COUNT(DISTINCT pan_no) investors FROM kfin_folio GROUP BY fund_code ORDER BY folios DESC", conn)
            if s.empty: st.info("No KFin folio data.")
            else: st.dataframe(s, use_container_width=True, hide_index=True)
            ...
            st.divider()
            with get_conn() as conn:
                s = pd.read_sql("SELECT trade_date,fund_code,COUNT(*) txns,COUNT(DISTINCT folio_no) folios,ROUND(SUM(amount),2) amount FROM kfin_transactions GROUP BY trade_date,fund_code ORDER BY trade_date DESC LIMIT 30", conn)
            if s.empty: st.info("No KFin transaction data.")
            else: st.dataframe(s, use_container_width=True, hide_index=True)
            ...
            st.divider()
            with get_conn() as conn:
                s = pd.read_sql("SELECT fund_code,status,COUNT(*) sips,COUNT(DISTINCT folio_no) folios,ROUND(SUM(amount),2) amount FROM kfin_sip GROUP BY fund_code,status ORDER BY fund_code", conn)
            if s.empty: st.info("No KFin SIP data.")
            else: st.dataframe(s, use_container_width=True, hide_index=True)
