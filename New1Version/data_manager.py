...
def parse_bse_client_master(file, replace: bool) -> tuple[bool, str]:
    ...
    for _, row in df.iterrows():
        ...
        raw_val(row.get(s3_pan_col, "")) if s3_pan_col else "", raw_val(row.get(g_pan_col, "")) if g_pan_col else "")
        ...
        raw_val(row.get(s3_ckyc_col, "")) if s3_ckyc_col else "", raw_val(row.get(g_kyc_col, "")) if g_kyc_col else "",
...
def parse_bse_sip(file, replace: bool) -> tuple[bool, str, dict]:
    ...
    with get_conn() as conn:
        if replace:
            conn.execute("DELETE FROM bse_sip"); conn.executemany(insert_sql, rows); inserted = len(rows)
        else:
            before = _count_before_after(conn, "bse_sip"); conn.executemany(insert_sql,
                                                                            rows); inserted, dupes = _inserted_dupes(
    ...
    if skipped: msg += f" | Skipped {skipped}"; if
    dupes: msg += f" | {dupes} duplicates"
...
def parse_bse_scheme_master(file, replace: bool) -> tuple[bool, str, dict]:
    ...
    for _, row in df.iterrows():
        ...
        try:
            ...
            log.warning("Skipped scheme row: %s", exc); skipped += 1
    ...
    with get_conn() as conn:
        if replace:
            conn.execute("DELETE FROM bse_scheme_master"); conn.executemany(sql, rows); inserted = len(rows)
        else:
            before = _count_before_after(conn, "bse_scheme_master"); conn.executemany(sql,
                                                                                      rows); inserted, dupes = _inserted_dupes(
...
def _execute_cams_import(table: str, rows: list, batch: str, replace: bool, skipped: int) -> tuple[bool, str, dict]:
    ...
    with get_conn() as conn:
        if replace:
            conn.execute(f"DELETE FROM {table}"); conn.executemany(sql, rows); inserted = len(rows)
        else:
            before = _count_before_after(conn, table); conn.executemany(sql, rows); inserted, dupes = _inserted_dupes(
