...
def init_db() -> None:
    ...
    with get_conn() as conn:
        ...
        try:
            conn.execute(
                "CREATE UNIQUE INDEX IF NOT EXISTS ux_cams_aum "
                "ON cams_aum (folio_no, scheme_name, rep_date)"
            )
        except sqlite3.OperationalError:
            pass
        ...
        try:
            conn.execute(
                "CREATE UNIQUE INDEX IF NOT EXISTS ux_kfintech_aum "
                "ON kfintech_aum (folio_no, scheme_name, rep_date)"
            )
        except sqlite3.OperationalError:
            pass
...
def get_next_sip_date(day) -> str:
    ...
    try:
        ...
        try:
            ...
            last_day = (
                    next_month_first.replace(month=next_month_first.month % 12 + 1, day=1)
                    - timedelta(days=1)
            )
...
def _amfi_sync_worker(result_bucket: list) -> None:
    try:
        ...
        if schemes:
            with get_conn() as conn:
                conn.executemany(
                    "INSERT OR REPLACE INTO amc_schemes VALUES (?,?,?,?,?,?,?)", schemes
                )
                conn.execute(
...
def parse_client_master(file, replace: bool) -> tuple[bool, str]:
    ...
    with get_conn() as conn:
        if replace:
            # Replace mode: delete all existing clients, then insert fresh
            ...
            # Smart upsert: insert new, skip existing (by client_code)
...
def parse_sip_report(file, replace: bool) -> tuple[bool, str, dict]:
    ...
    for _, row in df.iterrows():
        ...
        try:
            ...
            rows.append((
                clean_str(row["client_code"]),
                clean_str(row["folio_no"]),
                clean_str(row.get("rta_scheme_code", "")),
                clean_str(row["scheme_name"]),
                amc_raw,
                get_rta(amc_raw),
                clean_str(row.get("frequency_type", "")),
                float(row.get("installments_amt") or 0),
                sip_day,
                sd,
                parse_date_safe(row.get("end_date")),
                "Active",
                fo,
            ))
        except Exception as exc:
    ...
    with get_conn() as conn:
        if replace:
            # Replace mode: wipe holdings then reinsert all
            conn.execute("DELETE FROM holdings")
            conn.executemany(
                "INSERT INTO holdings "
                "(client_code, folio_no, scheme_code, scheme_name, amc, rta, "
            ...
            # Smart upsert: check folio_no + scheme_code + start_date uniqueness
            # Build a set of existing (folio_no, scheme_code, start_date) tuples
            existing = set(
                conn.execute(
            ...
            for r in rows:
                key = (
                    normalize_folio(r[1]),  # folio_no
                    str(r[2]).strip().lower(),  # scheme_code
                    r[9],  # start_date
                )
                if key in existing:
                    duplicate_skipped += 1
                    ...
                    new_rows.append(r)
                    existing.add(key)  # avoid dupes within same upload batch
...
def parse_cams_brokerage(file, replace: bool) -> tuple[bool, str, dict]:
    ...
    if df is None:
        return False, f"Could not parse file — {last_err}", {}

    df.columns = [
        col.strip().replace('\u200b', '').replace('\ufeff', '').replace('\u00a0', '').strip('\'"').upper()
    ...
    missing = [c for c in required if c not in df.columns]
    if missing:
        log.warning("CAMS columns found: %s", list(df.columns)[:20])
        return (
            False,
            f"Missing columns: {missing}. Found: {list(df.columns)[:15]}",
            {},
        )
    ...
    for _, row in df.iterrows():
        try:
            rows.append((
                ...
                _safe_float(row.get("BRKAGE_RATE", 0)),
                clean_str(row.get("INV_NAME", "")).strip(),
                parse_date_safe(row.get("PROC_DATE", "")),
                _parse_accrual_month(row.get("BROKERAGE_ACRUAL_MONTH", "")),
                _safe_float(row.get("PLOT_AMOUNT", 0)),
                _safe_float(row.get("AVG_ASSETS", 0)),
                _safe_float(row.get("IGST_VALUE", 0)),
                _safe_float(row.get("CGST_VALUE", 0)),
                _safe_float(row.get("SGST_VALUE", 0)),
                batch_id,
            ))
        except Exception as exc:
    ...
    with get_conn() as conn:
        if replace:
            # Replace mode: wipe only CAMS brokerage, reinsert all
            ...
            # Smart upsert: INSERT OR IGNORE skips existing (trxn_no, folio_no, accrual_month) dupes
...
def parse_cams_aum(file, replace: bool) -> tuple[bool, str, dict]:
    ...
    if df is None:
        return False, f"Could not parse AUM file — {last_err}", {}
    ...
    for _, row in df.iterrows():
        ...
        try:
            rows.append((
                ...
                _safe_float(row.get("CLOS_BAL", 0)),
                _safe_float(row.get("RUPEE_BAL", 0)),
                batch_id,
            ))
        except Exception as exc:
    ...
    with get_conn() as conn:
        if replace:
            ...
            conn.executemany(
                ...
                "(folio_no, inv_name, scheme_name, amc_code, pan_no, email, "
                " rep_date, units, rupee_bal, upload_batch) "
                "VALUES (?,?,?,?,?,?,?,?,?,?)",
                rows,
            )
            inserted = len(rows)
...
def parse_kfintech_aum(file, replace: bool) -> tuple[bool, str, dict]:
    """Parse KFinTech (Karvy) AUM report CSV/TSV."""
    df = None
    last_err = ""
    ...
    if df is None:
        return False, f"Could not parse KFinTech AUM file — {last_err}", {}
    ...
    for key, candidates in col_mappings.items():
        resolved[key] = _find_col(candidates)
    ...
    for _, row in df.iterrows():
        ...
        try:
            ...
            rows.append((
                ...
                clean_str(row.get(resolved['amc_code'], "")),
                clean_str(row.get(resolved['product_code'], "")),
                clean_str(row.get(resolved['scheme_code'], "")),
                clean_str(row.get(resolved['dividend_opt'], "")),
                clean_str(row.get(resolved['email'], "")).lower(),
                parse_date_safe(row.get(resolved['rep_date'], "")),
                _safe_float(row.get(resolved['units'], 0)) if resolved['units'] else 0.0,
                aum_val,
                _safe_float(row.get(resolved['nav'], 0)) if resolved['nav'] else 0.0,
                aum_val,
                batch_id,
            ))
        except Exception as exc:
    ...
    with get_conn() as conn:
        if replace:
            ...
            conn.executemany(
                "INSERT OR IGNORE INTO kfintech_aum "
                "(folio_no, inv_name, scheme_name, amc_code, product_code, scheme_code, "
                " dividend_opt, email, rep_date, units, rupee_bal, nav, aum, upload_batch) "
                "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                rows,
            )
            inserted = len(rows)
...
def load_client_aum(client_code: str) -> pd.DataFrame:
    with get_conn() as conn:
        client_folios = conn.execute(
            "SELECT folio_no FROM holdings WHERE client_code = ?", (client_code,)
        ).fetchall()
        pan_row = conn.execute(
    ...
    with get_conn() as conn:
        aum_df = pd.read_sql(
            "SELECT folio_no, inv_name, scheme_name, amc_code, rupee_bal, units, rep_date, pan_no FROM cams_aum",
            conn,
        )
    ...
    if client_pan:
        ...
        pan_rows = aum_df[
            (aum_df["pan_clean"] == client_pan) &
            (~aum_df["folio_no"].str.strip().str.lower().isin(already_folios))
            ].copy()
        pan_rows["match_type"] = "Lumpsum (PAN)"
        pan_matched = pan_rows

    result = pd.concat([folio_matched, pan_matched], ignore_index=True)
    result = result.drop(columns=["folio_base", "pan_clean", "pan_no"], errors="ignore")
    return result
...
def load_client_kfintech_aum(client_code: str) -> pd.DataFrame:
    with get_conn() as conn:
        ...
        client_row = conn.execute(
            "SELECT pan, email FROM clients WHERE client_code = ?", (client_code,)
        ).fetchone()
        client_email = client_row[1].strip().lower() if client_row and client_row[1] else ""

    with get_conn() as conn:
        aum_df = pd.read_sql(
            "SELECT folio_no, inv_name, scheme_name, amc_code, product_code, scheme_code, "
            "dividend_opt, rupee_bal, units, nav, rep_date, email FROM kfintech_aum",
            conn,
        )
    ...
    if client_email:
        already_folios = set(folio_matched["folio_no"].str.strip().str.lower())
        email_rows = aum_df[
            (aum_df["email_clean"] == client_email) &
            (~aum_df["folio_no"].str.strip().str.lower().isin(already_folios))
            ].copy()
        if not email_rows.empty:
            email_rows["match_type"] = "Lumpsum (Email) (KFinTech)"
            email_matched = email_rows

    result = pd.concat([folio_matched, email_matched], ignore_index=True)
    result = result.drop(columns=["folio_base", "email_clean"], errors="ignore")
    return result


@st.cache_data(ttl=60, show_spinner=False)
def load_combined_client_aum(client_code: str) -> pd.DataFrame:
    cams_df = load_client_aum(client_code)
    kfin_df = load_client_kfintech_aum(client_code)

    if not cams_df.empty:
        cams_df["rta_source"] = "CAMS"
    if not kfin_df.empty:
        kfin_df["rta_source"] = "KFinTech"

    all_cols = list(set(list(cams_df.columns) + list(kfin_df.columns)))
    for col in all_cols:
        if col not in cams_df.columns:
            cams_df[col] = None
        if col not in kfin_df.columns:
            kfin_df[col] = None

    combined = pd.concat([cams_df, kfin_df], ignore_index=True) if not (
            cams_df.empty and kfin_df.empty) else pd.DataFrame()
    return combined
...
def load_cams_by_client(client_code: str) -> pd.DataFrame:
    with get_conn() as conn:
        ...
        merged = cams_matched.merge(
            holdings_df[["folio_base", "scheme_name", "amc", "rta"]],
            on="folio_base", how="left"
        )
    ...
    result = merged.rename(columns={
        "brkage_amt": "brokerage",
        "plot_amount": "transaction_amount",
        "brkage_rate": "rate_pct"
    })

    expected_cols = [
        "accrual_month", "amc_name", "scheme_name", "folio_no",
        "trxn_type", "transaction_amount", "brokerage", "rate_pct", "rta"
    ]
...
def load_distinct_cams_amc_codes() -> list[str]:
    with get_conn() as conn:
        rows = conn.execute(
            "SELECT DISTINCT amc_code FROM cams_brokerage WHERE amc_code != '' ORDER BY amc_code"
        ).fetchall()
    return [r[0] for r in rows]
...
def load_notifications() -> dict:
    ...
    with get_conn() as conn:
        holdings = pd.read_sql(
            "SELECT h.client_code, c.name, h.scheme_name, h.sip_day "
            "FROM holdings h JOIN clients c ON h.client_code = c.client_code "
    ...
    with get_conn() as conn:
        aum_folios = conn.execute(
            ...
            "AND (pan_no IS NULL OR pan_no = '' OR pan_no NOT IN (SELECT pan FROM clients WHERE pan != ''))"
        ).fetchone()[0]
    ...
    with get_conn() as conn:
        active_amc_list = [
            r[0] for r in conn.execute(
                "SELECT DISTINCT amc FROM holdings WHERE status='Active'"
            ).fetchall()
        ]
        entered = {
            r[0] for r in conn.execute(
                "SELECT amc FROM monthly_brokerage WHERE month=? AND year=?",
...
def global_search(query: str) -> dict:
    ...
    with get_conn() as conn:
        clients_df = pd.read_sql(
            "SELECT client_code, name, pan FROM clients "
            "WHERE LOWER(name) LIKE ? OR UPPER(pan) LIKE ?",
            conn, params=(f"%{q}%", f"%{query.upper()}%"),
        )
        if not clients_df.empty:
            results["clients"] = clients_df.to_dict("records")

        folios_df = pd.read_sql(
            "SELECT DISTINCT h.folio_no, h.scheme_name, h.amc, c.name as client_name "
            "FROM holdings h LEFT JOIN clients c ON h.client_code = c.client_code "
            "WHERE LOWER(h.folio_no) LIKE ? OR LOWER(h.scheme_name) LIKE ?",
            conn, params=(f"%{q}%", f"%{q}%"),
        )
        if not folios_df.empty:
            results["folios"] = folios_df.to_dict("records")

        aum_df = pd.read_sql(
            "SELECT DISTINCT folio_no, inv_name, scheme_name, pan_no "
            "FROM cams_aum WHERE LOWER(folio_no) LIKE ? OR LOWER(scheme_name) LIKE ? OR LOWER(inv_name) LIKE ?",
            conn, params=(f"%{q}%", f"%{q}%", f"%{q}%"),
        )
        if not aum_df.empty:
            results["aum"] = aum_df.to_dict("records")
    return results
...
<style>
    ...
    .streamlit-expanderContent, .stAlert { color: var(--text-color) !important; }
    ...
    .search-card {
        background: var(--secondary-background-color); border:1px solid var(--border-color); border-radius:8px;
        padding:10px 14px; margin-bottom:6px;
    }
    .search-tag {
        display:inline-block; font-size:0.7rem; font-weight:600;
        background: var(--accent-color); color: var(--accent-text); border-radius:4px;
...
<style>
    ...
    .search-card {{
        background:{_sbg}; border:1px solid {_border}; border-radius:8px;
        padding:10px 14px; margin-bottom:6px;
    }}
    .search-tag {{
        display:inline-block; font-size:0.7rem; font-weight:600;
        background:{_accent}22; color:{_accent}; border-radius:4px;
        padding:1px 6px; margin-right:6px;
    }}
</style>
""", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    if search_q and len(search_q.strip()) >= 2:
        ...
        if not sr:
            ...
            if "clients" in sr:
                ...
                for r in sr["clients"][:5]:
                    st.markdown(
                        f'<div class="search-card"><span class="search-tag">Client</span>'
                        f'<b>{r["name"]}</b><br><small style="color:{_muted}">{r["pan"] or "—"}</small></div>',
                        unsafe_allow_html=True,
                    )
            if "folios" in sr:
                st.markdown(f"**📂 Folios / Schemes** ({len(sr['folios'])})")
                for r in sr["folios"][:5]:
                    st.markdown(
                        f'<div class="search-card"><span class="search-tag">Folio</span>'
                        f'<b>{r["folio_no"]}</b><br>'
                        f'<small style="color:{_muted}">{r["scheme_name"][:40]} — {r["client_name"] or "?"}</small'
                        f'></div>',
                        unsafe_allow_html=True,
                    )
            if "aum" in sr:
                ...
                for r in sr["aum"][:4]:
                    st.markdown(
                        f'<div class="search-card"><span class="search-tag">AUM</span>'
                        f'<b>{r["inv_name"]}</b><br>'
                        f'<small style="color:{_muted}">{r["scheme_name"][:40]}</small></div>',
                        unsafe_allow_html=True,
                    )
        st.divider()
    ...
    mode = st.radio(
        "Navigate",
        ["📊 Dashboard", "👤 Client View", "💰 Earnings", "⚙️ Admin Panel"],
    ...
    if total_n:
        notif_label += f' <span class="notif-badge">{total_n}</span>'
...
if mode == "📊 Dashboard":
    ...
    total_aum = aum_data["total"]
    cams_aum = aum_data["cams"]
    kfin_aum = aum_data["kfintech"]
    ...
    if total_aum > 0:
        ...
        with aum_col1:
            st.markdown(
                f"""<div class="aum-card">
                    <div class="label">📦 Total AUM (All RTAs)</div>
                    <div class="value">{format_aum(total_aum)}</div>
                </div>""",
                unsafe_allow_html=True
            )
        with aum_col2:
            st.markdown(
                f"""<div class="aum-card" style="background: linear-gradient(135deg, #1a3a5c 0%, #2d6494 100%);">
                    <div class="label">📦 CAMS AUM</div>
                    <div class="value">{format_aum(cams_aum)}</div>
                </div>""",
                unsafe_allow_html=True
            )
        with aum_col3:
            st.markdown(
                f"""<div class="aum-card-kfin">
                    <div class="label">📦 KFinTech AUM</div>
                    <div class="value">{format_aum(kfin_aum)}</div>
                </div>""",
                unsafe_allow_html=True
            )
        with aum_col4:
            with get_conn() as conn:
                aum_schemes = conn.execute(
                    "SELECT COUNT(DISTINCT scheme_name) FROM (SELECT scheme_name FROM cams_aum UNION SELECT "
                    "scheme_name FROM kfintech_aum)"
                ).fetchone()[0]
                aum_folios = conn.execute(
                    "SELECT COUNT(DISTINCT folio_no) FROM (SELECT folio_no FROM cams_aum UNION SELECT folio_no FROM "
                    "kfintech_aum)"
                ).fetchone()[0]
            st.markdown(
                f"""<div class="aum-card" style="background: linear-gradient(135deg, #5c1a3a 0%, #942d64 100%);">
                    <div class="label">📂 Schemes × Folios</div>
                    <div class="value">{aum_schemes} × {aum_folios}</div>
                </div>""",
                unsafe_allow_html=True
            )
    else:
    ...
    if df_filtered.empty:
        ...
        with c2:
            fig = px.bar(amc_grp, x="amc", y="sip_amount",
...
# ==================== 👤 CLIENT VIEW ====================
    ...
    if clients.empty:
        ...
        if sel:
            ...
            if rta_filter != "All":
                c_holdings = c_holdings[c_holdings["rta"] == rta_filter]
            ...
            cams_aum_df = load_client_aum(code)
            kfin_aum_df = load_client_kfintech_aum(code)
            ...
            has_sips = not c_holdings.empty
            has_aum = not client_aum_df.empty
            has_cams = not cams_aum_df.empty
            has_kfin = not kfin_aum_df.empty
            has_lumpsum = (
                    has_aum and
                    "match_type" in client_aum_df.columns and
            ...
            if not has_sips and not has_aum:
                st.info(f"No active SIPs or AUM data found for **{sel}**.")

            if has_lumpsum and not has_sips:
                st.warning(
                    "⚠️ **Lumpsum-only client** — no active SIPs found. "
                    "AUM below is matched via PAN/Email from AUM reports.",
                    icon=None,
                )
            elif has_lumpsum:
                st.info(
                    "ℹ️ This client also has lumpsum holdings matched via PAN/Email "
                    "(shown with **Lumpsum** tag in AUM breakdown)."
                )
            ...
            if has_aum:
                with st.expander("📦 AUM Holdings Breakdown", expanded=(not has_sips)):
                    ...
                    if "rupee_bal" in aum_display.columns:
                        aum_display["rupee_bal"] = aum_display["rupee_bal"].apply(
                            lambda x: format_currency(x, decimals=2)
                        )
                    if "units" in aum_display.columns:
                        aum_display["units"] = aum_display["units"].apply(
                    ...
                    if len(client_aum_df) > 1:
                        fig_pie = px.pie(
                            client_aum_df, values="rupee_bal", names="scheme_name",
            ...
            if has_sips:
                ...
                display_df = c_holdings[[
                    "scheme_name", "amc", "rta", "folio_no", "sip_amount", "start_date", "next_sip", "first_order"
                ]].copy()
                ...
                st.dataframe(
                    display_df.iloc[start:end][[
                        "scheme_name", "amc", "rta", "folio_no", "SIP Amt", "Start Date", "next_sip", "First Order"
                    ]].rename(columns={
                        "scheme_name": "Scheme", "amc": "AMC", "rta": "RTA",
                        "folio_no": "Folio", "next_sip": "Next SIP",
                    }),
                    use_container_width=True, hide_index=True,
                )

                csv = c_holdings.to_csv(index=False).encode()
                st.download_button("⬇️ Export Client Holdings (CSV)", csv,
                                   f"{sel}_holdings.csv", "text/csv")

            st.divider()
            st.subheader("💹 Brokerage Generated (from CAMS File)")
            cams_client = load_cams_by_client(code)
            if cams_client.empty:
                st.info("No CAMS brokerage data found for this client's folios.")
            else:
                if rta_filter != "All":
                    cams_client = cams_client[cams_client["rta"] == rta_filter]

                months_avail = sorted(cams_client["accrual_month"].unique(), reverse=True)
                sel_month = st.selectbox(
                    "📅 Filter by Accrual Month",
                    ["All"] + months_avail,
                    key=f"cams_filter_{code}"
                )

                view_df = cams_client if sel_month == "All" else cams_client[cams_client["accrual_month"] == sel_month]
                total_cams = view_df["brokerage"].sum()
                ...
                disp = view_df[["accrual_month", "amc_name", "scheme_name", "folio_no",
                                "trxn_type", "transaction_amount", "brokerage", "rate_pct", "rta"]].copy()
                ...
                disp["rate_pct"] = disp["rate_pct"].apply(
                    lambda x: f"{float(x):.4f}%" if x else "-"
                )
                disp.columns = ["Month", "AMC", "Scheme", "Folio", "Txn Type",
                                "Txn Amount", "Brokerage", "Rate", "RTA"]
                st.dataframe(disp, use_container_width=True, hide_index=True)

                cams_csv = view_df.to_csv(index=False).encode()
                st.download_button("⬇️ Export Client Brokerage (CSV)", cams_csv,
                                   f"{sel}_brokerage.csv", "text/csv")
...
# ==================== 💰 EARNINGS ====================
    ...
    if rta_filter != "All":
        client_amcs = [amc for amc in client_amcs if df_active[df_active["amc"] == amc]["rta"].iloc[0] == rta_filter]
    ...
    if not client_amcs:
                ...
                if not sel_amc:
                    ...
                    with get_conn() as conn:
                        conn.execute(
                            ...
                            "VALUES (?,?,?,?,?) "
                            "ON CONFLICT(amc, month, year) DO UPDATE SET amount=excluded.amount, notes=excluded.notes",
        ...
        cams_months = load_cams_months()
        curr_ym = f"{current_year}-{datetime.now().month:02d}"
        if curr_ym not in cams_months:
            cams_months.append(curr_ym)
        cams_months = sorted(cams_months, reverse=True)

        sel_ym = st.selectbox(
            "📅 Select Reconciliation Month",
            cams_months,
            format_func=lambda x: datetime(*map(int, x.split("-")), 1).strftime("%B %Y"),
            index=cams_months.index(curr_ym)
        )
        ...
        for amc in client_amcs:
            ...
            diff = manual - cams_val if (manual and cams_val) else None
            ...
            table_data.append({
                "AMC": amc,
                "SIP Count": sip_ct,
                "Monthly Volume": format_currency(sip_vol, decimals=0),
        ...
        tm2.metric("📄 Total CAMS File", format_brokerage(total_cams_sum))
        tm3.metric("↔️ Difference", format_brokerage(total_manual - total_cams_sum))

        all_cams_months = load_cams_months()
        if all_cams_months:
            st.divider()
            st.subheader("📅 CAMS Brokerage — By Accrual Month")
            sel_cams_month = st.selectbox("Accrual Month", all_cams_months, key="earnings_cams_month")
            month_df = load_cams_by_amc(sel_cams_month)
            if not month_df.empty:
                month_df["cams_brokerage"] = month_df["cams_brokerage"].apply(format_brokerage)
                st.dataframe(
                    month_df.rename(columns={
                        "amc_code": "AMC Code", "amc_name": "AMC Name",
                        "cams_brokerage": "Brokerage", "folio_count": "Folios",
                        "accrual_month": "Month",
                    }),
                    use_container_width=True, hide_index=True,
                )
        ...
        st.download_button("⬇️ Export Earnings (CSV)", csv,
                           f"brokerage_{sel_month_str}_{sel_year}.csv", "text/csv")
        ...
        for amc in sorted(set(list(manual_map.keys()) + list(file_map.keys()) + client_amcs)):
            ...
            breakdown_data.append({
                ...
                "Months with File": len([m for m in load_cams_months() if amc in (
        ...
        if breakdown_df.empty:
            ...
            st.download_button(
                "⬇️ Export AMC Breakdown (CSV)",
                csv,
                f"amc_breakdown_{datetime.now().strftime('%Y%m%d')}.csv",
                "text/csv"
            )
...
# ==================== ⚙️ ADMIN PANEL ====================
    ...
    # ── 5 tabs: Import BSE Data | Import RTA Data | AMC Config | AMC Code Map | DB Info | RTA Mapping ──
    ...
    with tab_bse:
        ...
        replace2 = st.checkbox(
            "Replace existing holdings",
            key="replace_holdings",
            help="Checked: deletes ALL existing holdings, then reinserts active SIPs from file.\n"
                 "Unchecked: only inserts records with a new (folio + scheme_code + start_date) combo; duplicates are "
                 "skipped.",
        )
        if replace2:
    ...
    with tab_rta:
        ...
        if rta_section == "📂 Brokerage Data":
            ...
            cams_file = st.file_uploader(
                "CAMS Brokerage CSV / TSV", type=["csv", "txt", "tsv"], key="cams_file"
            )
            replace_cams = st.checkbox(
                "Replace ALL existing CAMS brokerage data",
                key="replace_cams",
                help="Checked: deletes all existing CAMS brokerage rows, then reinserts.\n"
                     "Unchecked: only inserts transactions not already present (matched on trxn_no + folio + "
                     "accrual_month).",
            )
            if replace_cams:
            ...
            with get_conn() as conn:
                cams_summary = pd.read_sql(
                    ...
                    SELECT accrual_month AS "Month", COUNT(*) AS "Rows",
                           ...
                           ROUND(SUM(brkage_amt), 2) AS "Total Brokerage (Rs)",
                           upload_batch AS "Batch"
                    FROM cams_brokerage GROUP BY accrual_month, upload_batch ORDER BY accrual_month DESC
                    """, conn)
        ...
        # ── AUM ───────────────────────────────────────────────────────────────
            ...
            with col_cams:
                ...
                aum_file = st.file_uploader(
                    "CAMS AUM CSV / TSV", type=["csv", "txt", "tsv"], key="aum_file"
                )
                replace_aum = st.checkbox(
                    "Replace existing CAMS AUM",
                    key="replace_aum",
            ...
            with col_kfin:
                ...
                kfin_file = st.file_uploader(
                    "KFinTech AUM CSV / TSV", type=["csv", "txt", "tsv"], key="kfin_aum_file"
                )
                replace_kfin = st.checkbox(
                    "Replace existing KFinTech AUM",
                    key="replace_kfin",
            ...
            with c1:
                ...
                with get_conn() as conn:
                    aum_summary = pd.read_sql(
                        """
                        SELECT rep_date AS "Report Date", amc_code AS "AMC Code",
                               COUNT(*) AS "Folios",
                               ROUND(SUM(rupee_bal), 2) AS "Total AUM (Rs)"
                        FROM cams_aum GROUP BY rep_date, amc_code ORDER BY rep_date DESC, 4 DESC
                        """, conn)
                ...
                with get_conn() as conn:
                    kfin_summary = pd.read_sql(
                        ...
                        SELECT rep_date AS "Report Date", amc_code AS "AMC Code",
                               COUNT(*) AS "Folios",
                               ROUND(SUM(rupee_bal), 2) AS "Total AUM (Rs)"
                        FROM kfintech_aum GROUP BY rep_date, amc_code ORDER BY rep_date DESC, 4 DESC
                        """, conn)
    ...
    with tab_amc:
        ...
        if amc_cfg.empty:
            ...
            for _, row in amc_cfg.iterrows():
                updated_enabled[row["amc"]] = st.checkbox(
                    row["amc"], value=bool(row["is_enabled"]), key=f"amc_{row['amc']}"
                )
            if st.button("💾 Save AMC Config"):
                with get_conn() as conn:
                    for amc, enabled in updated_enabled.items():
                        conn.execute(
                            "UPDATE amc_config SET is_enabled=? WHERE amc=?",
                            (1 if enabled else 0, amc),
                        )
    ...
    with tab_map:
        ...
        with get_conn() as conn:
            known_amcs = sorted({
                r[0] for r in conn.execute(
                    "SELECT DISTINCT amc FROM holdings WHERE amc IS NOT NULL AND amc != ''"
                ).fetchall()
            })
        ...
        if not cams_codes:
            ...
            if unmapped:
                st.warning(f"{len(unmapped)} code(s) not yet mapped: {', '.join(unmapped)}")
            ...
            for row_i in range(rows_needed):
                ...
                for col_i in range(cols_per_row):
                    ...
                    with cols[col_i]:
                        sel = st.selectbox(
                            f"AMC Code: **{code}**",
                            options, index=default_idx, key=f"amc_map_{code}",
    ...
    with tab_rta_map:
        ...
        for row_i in range(rows_needed):
            ...
            for col_i in range(cols_per_row):
                idx = row_i * cols_per_row + col_i
                if idx >= len(amc_rta_map):
                    break
                amc = list(amc_rta_map.keys())[idx]
                current = amc_rta_map[amc]
                with cols[col_i]:
                    sel = st.selectbox(
                        amc, ["CAMS", "KFinTech", "Unknown"],
                        index=["CAMS", "KFinTech", "Unknown"].index(current),
                        key=f"rta_{amc}"
                    )
